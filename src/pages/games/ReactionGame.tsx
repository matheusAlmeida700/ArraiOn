...
const ReactionGame = () => {
  ...
  const getBackgroundColor = () => {
    switch (gameState) {
      ...
      case 'clicked':
        return currentReactionTime === -1 ? 'from-gray-400 to-gray-600' : 'from-blue-400 to-blue-600';
      default:
  ...
  if (gameState === 'ended') {
    const validTimes = reactionTimes.filter(time => time > 0);
    const averageTime = validTimes.length > 0 ? validTimes.reduce((a, b) => a + b, 0) / validTimes.length : 0;
    const points = averageTime > 0 ? Math.max(10, Math.floor(1000 / averageTime * 10) + validTimes.length * 2) : 0;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-400 via-red-500 to-yellow-500 p-4 flex items-center justify-center">
        <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-8 max-w-md w-full shadow-lg border-2 border-yellow-300 text-center">
          ...
          <h2 className="text-2xl font-bold text-orange-600 mb-4">Teste Finalizado!</h2>
          <p className="text-lg mb-2">Reflexos válidos: {validTimes.length}/{totalRounds}</p>
          {averageTime > 0 && (
            <p className="text-lg mb-2">Tempo médio: {Math.round(averageTime)}ms</p>
          )}
          <p className="text-xl font-bold text-green-600 mb-6">+{points} pontos ganhos! 🎉</p>
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${getBackgroundColor()} p-4 flex flex-col`}>
        ...
        >
          <div className="text-center text-white">
            <div className="text-8xl mb-8">
              {gameState === 'go' ? '🎆' : gameState === 'clicked' && currentReactionTime !== -1 ? '✨' : '⏳'}
            </div>
            <h2 className="text-2xl font-bold mb-4">{getMessage()}</h2>
            
            {gameState === 'waiting' && (
              <p className="text-lg opacity-80">Clique quando ver o foguete! 🎆</p>
            )}
            
            {gameState === 'clicked' && currentReactionTime !== null && currentReactionTime > 0 && (
              <div className="text-lg">
                <p>Tempo de reação: {currentReactionTime}ms</p>
                <p className="mt-2">
                  {currentReactionTime < 300 ? '🔥 Incrível!' : 
                   currentReactionTime < 500 ? '⚡ Muito bom!' :
                   currentReactionTime < 700 ? '👍 Bom!' : '🐌 Pode melhorar!'}
                </p>
              </div>
            )}
          </div>
