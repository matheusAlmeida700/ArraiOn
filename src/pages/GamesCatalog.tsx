...
const GamesCatalog = () => {
  ...
  return (
    <div className="min-h-screen festa-bg text-white festa-texture">
      <div className="max-w-7xl mx-auto p-4 lg:p-8">
        <PointsDisplay />
        
        {/* Hero Banner */}
        <div 
          className={`hero-banner rounded-3xl lg:rounded-[3rem] p-8 lg:p-16 mb-12 lg:mb-16 text-center transition-all duration-1000 ${isLoaded ? 'animate-slide-up' : 'opacity-0'}`}
          style={{
            backgroundImage: 'linear-gradient(135deg, rgba(139, 92, 246, 0.9), rgba(236, 72, 153, 0.8)), url(https://images.unsplash.com/photo-1465146344425-f00d5f5c8f07?w=1200)',
        ...
        >
          ...
          <p className="text-lg lg:text-3xl xl:text-4xl text-white font-medium mb-6">Escolha sua aventura e comece a diversão!</p>
          ...
          {filteredGames.map((game, index) => (
            ...
            >
              ...
              >
                ...
                {/* Game Icons */}
                <div className="absolute top-4 right-4 text-4xl lg:text-6xl group-hover:animate-tada">{game.thumbnail}</div>
                <div className="absolute bottom-4 right-4 text-2xl lg:text-3xl group-hover:animate-bounce">{game.icon}</div>
                ...
                {hoveredGame === game.id && (
                  <div className="absolute inset-0 bg-gradient-to-t from-purple-900/80 via-transparent to-transparent animate-slide-up">
                    <div className="absolute bottom-6 left-6 right-6 text-center">
                      <p className="text-white font-medium text-sm lg:text-base">Clique para jogar!</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="p-6 lg:p-8">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex flex-col items-end space-y-2">
                    <span className={`text-xs lg:text-sm font-bold px-3 py-1 rounded-full border ${getDifficultyColor(game.difficulty)}`}>
                      {game.difficulty}
                    </span>
                  </div>
                </div>

                <p className="text-purple-200 text-sm lg:text-base mb-6 leading-relaxed">{game.description}</p>
                ...
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-sm lg:text-base text-purple-300 font-medium">⏱️ {game.playTime}</span>
                </div>
        ...
        {/* Enhanced Navigation */}
        <div className={`grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6 transition-all duration-700 delay-700 ${isLoaded ? 'animate-slide-up' : 'opacity-0'}`}>
          <Button
